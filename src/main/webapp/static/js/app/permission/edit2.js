...
layui.use(['form', 'layedit', 'jacommon', 'laydate', 'layer','element','form'], function() {
	var form = layui.form(),
	   ...
	   var permissionVal = $('#permissionStr').val();
	   if (permissionVal !== 'undefined' && permissionVal !== undefined && permissionVal !== '' && permissionVal !== null) {
			$('#urlDiv').hide();
		} 
	...
	form.on('submit(btnSub)', function(data) {
		var postUrl = data.form.action,
			...
			var showFieldsStr = "";
			if (permissionVal !== 'undefined' && permissionVal !== '' && permissionVal !== null) {
				$('input[name="showFields"]:checked').each(function(){    
					showFieldsStr += $(this).val()+",";  
				});    
				postData.url = showFieldsStr;
			}
