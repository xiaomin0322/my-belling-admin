...
<html>
	...
	<body>
		<div style="margin: 15px;">
			...
			<form class="layui-form layui-form-pane" action="${base}/admin/permission/save" method="post">
				...
				<div class="layui-form-item" id="urlDiv">
					...
					<div class="layui-input-inline">
						<input type="text" name="url" value="${vo.url}"  placeholder="请输入URL" autocomplete="off" class="layui-input">
					</div>
				</div>
				
				
				<@getDict key="${permissionperParent.permission}">
								<#if val??>
								<input type="hidden" id="permissionStr" value="${val}" autocomplete="off" class="layui-input">
								<div class="layui-form-item">
								
										<label class="layui-form-label">显示字段</label>
										 <#list val?split(",") as t>  
										   <#assign fobj=(t?split(":"))/>
										   <#assign contFlag=(vo.url?contains(fobj[1]))/>
										   
										   <#if contFlag == true>
										        <input name="showFields" lay-skin="primary" title="${fobj[0]}" value="${fobj[1]}" checked="" type="checkbox">
										     <#else>
											    <input name="showFields" lay-skin="primary" title="${fobj[0]}" value="${fobj[1]}" type="checkbox">
											</#if>
		...
		<script type="text/javascript" src="${base}/static/js/app/permission/edit2.js"></script>
	</body>
</html>